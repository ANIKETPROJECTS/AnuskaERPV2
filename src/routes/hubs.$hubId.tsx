    ...
    >
      <div className="space-y-6 p-6">
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          <Panel title="Batch register" description="Traceable batches currently associated with this hub">
            {inventory.batches.length === 0 ? <Empty text="No batches recorded for this hub." /> : <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-sm"><thead className="border-b border-border text-left text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="px-5 py-3 font-medium">Batch</th><th className="px-5 py-3 font-medium">Item</th><th className="px-5 py-3 text-right font-medium">Received</th><th className="px-5 py-3 text-right font-medium">Available</th><th className="px-5 py-3 font-medium">Status</th></tr></thead><tbody>{inventory.batches.map((batch) => <tr key={batch.id} className="border-b border-border/70 last:border-0"><td className="tabular px-5 py-3 text-xs font-medium"><Link to="/inventory/batches/$batchId" params={{ batchId: batch.id }} className="text-primary hover:underline">{batch.batchCode}</Link></td><td className="px-5 py-3"><p>{batch.itemName}</p><p className="tabular text-xs text-muted-foreground">{batch.itemCode}</p></td><td className="tabular px-5 py-3 text-right">{num(batch.receivedQuantity + batch.producedQuantity)}</td><td className="tabular px-5 py-3 text-right font-semibold">{num(batch.availableQuantity)}</td><td className="px-5 py-3"><Tag tone={batch.availableQuantity ? "good" : "neutral"}>{batch.status}</Tag></td></tr>)}</tbody></table></div>}
          </Panel>
