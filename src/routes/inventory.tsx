...
export function InventoryManagement({ initialView }: { initialView: View }) {
  ...
  const [sortKey, setSortKey] = useState<"name" | "category" | "quantity" | "price" | "loggedAt">("name");
...
function InventoryTable({ items, totalItems, query, setQuery, categoryFilter, setCategoryFilter, stockFilter, setStockFilter, sortKey, setSortKey, sortDirection, setSortDirection, page, pageSize, setPage, setPageSize, loading }: { items: InventoryItem[]; totalItems: number; query: string; setQuery: (value: string) => void; categoryFilter: "all" | InventoryItem["category"]; setCategoryFilter: (value: "all" | InventoryItem["category"]) => void; stockFilter: "all" | "available" | "empty"; setStockFilter: (value: "all" | "available" | "empty") => void; sortKey: "name" | "category" | "quantity" | "price" | "loggedAt"; setSortKey: (value: "name" | "category" | "quantity" | "price" | "loggedAt") => void; sortDirection: "asc" | "desc"; setSortDirection: (value: "asc" | "desc") => void; page: number; pageSize: number; setPage: (value: number) => void; setPageSize: (value: number) => void; loading: boolean }) {
  return <Panel title="Inventory" description="Only records belonging to this SubHub workspace are shown." action={<Link to="/inventory/history" className="text-xs font-medium text-primary hover:underline">View movement history →</Link>}>
    <div className="flex flex-wrap items-end gap-3 border-b border-border p-4">
      <div className="relative min-w-[220px] flex-1 text-xs font-medium text-muted-foreground">Search raw materials<label className="relative mt-1 block"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Name, code, or category" className="h-9 w-full rounded-md border border-input pl-9 pr-3 text-sm font-normal text-foreground outline-none focus:border-primary" /></label></div>
      <label className="text-xs font-medium text-muted-foreground">Category<select value={categoryFilter} onChange={(event) => setCategoryFilter(event.target.value as "all" | InventoryItem["category"])} className="mt-1 h-9 rounded-md border border-input bg-white px-2 text-sm font-normal text-foreground"><option value="all">All categories</option><option value="Purchased">Purchased</option><option value="Molded">Molded</option></select></label>
