...
export async function replaceSubhubQualityHistoryWithDemoData(): Promise<
  { ok: true; count: number } | { ok: false; message: string }
> {
  if (!import.meta.env.DEV) {
    return { ok: false, message: "Demo history replacement is only available in development." };
  }

  const user = await getCurrentUserRecord("subhub");
  if (user?.panel !== "subhub" || user.role !== "subhub") {
    return { ok: false, message: "Only SubHub Managers can replace workspace quality history." };
  }
  ...
  const sampleRows: Array<{
    ...
    { code: "DEMO-RM-001", product: "Polypropylene Resin", category: "Raw Material", issue: "Faulty", quantity: 8, beforeQuantity: 120, daysAgo: 1 },
    { code: "DEMO-RM-002", product: "Rubber Seal", category: "Raw Material", issue: "Damaged", quantity: 3, beforeQuantity: 64, daysAgo: 2 },
    { code: "DEMO-FG-001", product: "Ball Float Body", category: "Float", issue: "Rejected", quantity: 2, beforeQuantity: 40, daysAgo: 3 },
    { code: "DEMO-FG-002", product: "Cover Cap", category: "Float", issue: "Expired", quantity: 5, beforeQuantity: 90, daysAgo: 6 },
    { code: "DEMO-RM-003", product: "Stainless Steel Pin", category: "Raw Material", issue: "Other", quantity: 4, beforeQuantity: 75, daysAgo: 10 },
    { code: "DEMO-FG-003", product: "Float Arm", category: "Float", issue: "Faulty", quantity: 1, beforeQuantity: 32, daysAgo: 15 },
  ];
  const demoLogs: QualityLogDocument[] = sampleRows.map((row) => ({
    _id: `demo-qmh-${randomUUID()}`,
    ...
    afterQuantity: row.beforeQuantity - row.quantity,
    notes: "DEMO SAMPLE ROW — not a real quality record.",
    recordedBy: "demo",
    recordedByName: "Demo data",
    createdAt: new Date(now.getTime() - row.daysAgo * 24 * 60 * 60 * 1000),
  }));

  const session = (await getMongoClient()).startSession();
  try {
    await session.withTransaction(async () => {
      const collection = workspaceDb.collection<QualityLogDocument>("quality_logs");
      await collection.deleteMany({}, { session });
      await collection.insertMany(demoLogs, { session });
    });
  } catch (error) {
    return {
      ok: false,
      message: error instanceof Error
        ? `Quality history was not replaced: ${error.message}`
        : "Quality history was not replaced.",
    };
  } finally {
    await session.endSession();
  }

  return { ok: true, count: demoLogs.length };
}
